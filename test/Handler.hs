...
prop_getWithQuery = HH.withTests 1 . HH.property $ do
  let
    request =
      WaiTest.setPath Wai.defaultRequest "/test/get_with_query?queryParam=queryValue"
...
prop_getWithQueryError = HH.withTests 1 . HH.property $ do
  ...
  HH.evalIO . WaiTest.withSession basicApp $ do
    ...
    WaiTest.assertBody "{\"bad_request\":\"Required query param missing: queryParam\"}" response
