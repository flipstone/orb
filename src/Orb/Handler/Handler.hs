...
import Beeline.HTTP.Client.ParameterCollectionSchema qualified as PCS
...
data RequestQuery query tags where
  RequestQuery ::
    ...
    (forall q. PCS.ParameterCollectionSchema q => q query query) ->
...
data RequestHeaders query tags where
  RequestHeaders ::
    Response.Has400Response tags =>
    (forall q. PCS.ParameterCollectionSchema q => q query query) ->
    RequestHeaders query tags
  EmptyRequestHeaders ::
...
readHeaders handler = do
  ...
  case requestHeaders handler of
    ...
    RequestHeaders schema ->
      case PCS.decodeHeaders schema (Wai.requestHeaders request) of
...
readQuery handler = do
  ...
  case requestQuery handler of
    ...
    RequestQuery schema ->
      case PCS.decodeQuery schema (Wai.rawQueryString request) of
        Left err -> fmap Left . Response.return400 . Response.BadRequestMessage $ err
        Right query -> pure . Right $ query
